...
@Component({
    selector: 'app-settings-observatory',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
    ...
    template: `
        <div class="card" dir="rtl">
            ...
            <div class="flex flex-col gap-2 mb-6">
                <div class="text-xl font-semibold text-surface-900 dark:text-surface-0">المرصد</div>
                <div class="text-surface-600 dark:text-surface-300 text-sm">إدارة المرصد (إضافة / تعديل / حذف).</div>
            </div>
            ...
            <p-toolbar styleClass="mb-6">
                <ng-template #start>
                    <p-button label="اضافة مرصد" icon="pi pi-plus" severity="secondary" (onClick)="openNew()" />
                </ng-template>
                <ng-template #end>
                    <p-iconfield>
            ...
            >
                ...
                <ng-template #emptymessage>
                    <tr>
                        <td colspan="3" class="text-center text-surface-600 dark:text-surface-300">لا يوجد مرصد.</td>
                    </tr>
                </ng-template>
            </p-table>
            ...
            <p-dialog
                ...
                [resizable]="false"
                [style]="{ width: '48rem' }"
                header="نافذة اضافة اسم المرصد"
            >
                <div class="flex flex-col gap-4" dir="rtl">
                    <div>
                        <label class="block font-medium text-surface-900 dark:text-surface-0 mb-2">اسم المرصد</label>
...
export class SettingsObservatory {
    @ViewChild('dt') dt?: Table;

    items = signal<Observatory[]>([
        { id: 1, name: 'مرصد 1' },
        { id: 2, name: 'مرصد 2' }
    ]);
    ...
    selected: Observatory = { id: 0, name: '' };
    ...
    save(): void {
        ...
        if (this.selected.id) {
            ...
            this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تم تحديث المرصد.' });
        } else {
            const nextId = this.nextId(this.items());
            this.items.update((list) => [...list, { id: nextId, name }]);
            this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تمت إضافة المرصد.' });
        }

        this.itemDialog = false;
    }

    confirmDelete(row: Observatory): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تم حذف المرصد.' });
            }
        });
    }
    ...
    private nextId(items: Observatory[]): number {
        let maxId = 0;
