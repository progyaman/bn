    ...
    template: `
        <div class="card" dir="rtl">
            ...
            <div class="flex flex-col gap-2 mb-6">
                <div class="text-xl font-semibold text-surface-900 dark:text-surface-0">الدولة</div>
                <div class="text-surface-600 dark:text-surface-300 text-sm">إدارة الدول (إضافة / تعديل / حذف).</div>
            </div>
            ...
            <p-toolbar styleClass="mb-6">
                <ng-template #start>
                    <p-button label="اضافة دولة" icon="pi pi-plus" severity="secondary" (onClick)="openNew()" />
                </ng-template>
                <ng-template #end>
                    <p-iconfield>
            ...
            >
                ...
                <ng-template #emptymessage>
                    <tr>
                        <td colspan="3" class="text-center text-surface-600 dark:text-surface-300">لا يوجد دول.</td>
                    </tr>
                </ng-template>
            </p-table>
            ...
            <p-dialog
                ...
                [resizable]="false"
                [style]="{ width: '48rem' }"
                header="نافذة اسم الدولة"
            >
                <div class="flex flex-col gap-4" dir="rtl">
                    <div>
                        <label class="block font-medium text-surface-900 dark:text-surface-0 mb-2">اسم الدولة</label>
...
export class SettingsCountry {
    ...
    save(): void {
        ...
        if (this.selected.id) {
            ...
            this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تم تحديث الدولة.' });
        } else {
            const nextId = this.nextId(this.items());
            this.items.update((list) => [...list, { id: nextId, name }]);
            this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تمت إضافة الدولة.' });
        }
    ...
    confirmDelete(row: Country): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تم حذف الدولة.' });
            }
        });
    }
    ...
    private nextId(items: Country[]): number {
        let maxId = 0;
