...
@Component({
    selector: 'app-settings-target-type',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
    ...
    template: `
        <div class="card" dir="rtl">
            ...
            <div class="flex flex-col gap-2 mb-6">
                <div class="text-xl font-semibold text-surface-900 dark:text-surface-0">تاركت</div>
                <div class="text-surface-600 dark:text-surface-300 text-sm">إدارة التاركت (إضافة / تعديل / حذف).</div>
            </div>
            ...
            <p-toolbar styleClass="mb-6">
                <ng-template #start>
                    <p-button label="اضافة تاركت" icon="pi pi-plus" severity="secondary" (onClick)="openNew()" />
                </ng-template>
                <ng-template #end>
                    <p-iconfield>
            ...
            >
                ...
                <ng-template #emptymessage>
                    <tr>
                        <td colspan="3" class="text-center text-surface-600 dark:text-surface-300">لا يوجد تاركت.</td>
                    </tr>
                </ng-template>
            </p-table>
            ...
            <p-dialog
                ...
                [resizable]="false"
                [style]="{ width: '48rem' }"
                header="نافذة اضافة تاركت"
            >
                <div class="flex flex-col gap-4" dir="rtl">
                    <div>
                        <label class="block font-medium text-surface-900 dark:text-surface-0 mb-2">اسم التاركت</label>
...
export class SettingsTargetType {
    @ViewChild('dt') dt?: Table;

    items = signal<TargetType[]>([
        { id: 1, name: 'تاركت 1' },
        { id: 2, name: 'تاركت 2' }
    ]);
    ...
    save(): void {
        ...
        if (this.selected.id) {
            ...
            this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تم تحديث التاركت.' });
        } else {
            const nextId = this.nextId(this.items());
            this.items.update((list) => [...list, { id: nextId, name }]);
            this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تمت إضافة التاركت.' });
        }

        this.itemDialog = false;
    }

    confirmDelete(row: TargetType): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.messageService.add({ severity: 'success', summary: 'تم', detail: 'تم حذف التاركت.' });
            }
        });
    }
    ...
    private nextId(items: TargetType[]): number {
        let maxId = 0;
