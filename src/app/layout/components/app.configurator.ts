...
@Component({
    ...
    template: `
        <p-drawer [visible]="visible()" (onHide)="onDrawerHide()" position="right" [transitionOptions]="'.3s cubic-bezier(0, 0, 0.2, 1)'" styleClass="layout-config-sidebar w-80" header="الإعدادات">
            <div class="flex flex-col gap-6">
                <div>
                    <span class="text-lg text-muted-color font-semibold">اللون الأساسي</span>
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        <span class="text-lg text-muted-color font-semibold">القوالب الجاهزة</span>
                        <p-selectbutton [options]="presets" [ngModel]="selectedPreset()" (ngModelChange)="onPresetChange($event)" [allowEmpty]="false"></p-selectbutton>
                    </div>
                </div>
                <div>
                    <div class="flex flex-col gap-2">
                        <span class="text-lg text-muted-color font-semibold">نمط الألوان</span>
                ...
                <!-- <div>
                    <div class="flex flex-col gap-2">
                        <span class="text-lg text-muted-color font-semibold">الاتجاه</span>
                        <div class="flex gap-3">
                            <button
                                type="button"
                                class="px-4 py-2 rounded border border-gray-300 bg-white hover:border-blue-500 text-sm"
                                [class.border-blue-600]="currentDirection() === 'rtl'"
                                [attr.aria-pressed]="currentDirection() === 'rtl'"
                                (click)="changeDirection('rtl')"
                            >RTL</button>
                            <button
                                type="button"
                                class="px-4 py-2 rounded border border-gray-300 bg-white hover:border-blue-500 text-sm"
                                [class.border-blue-600]="currentDirection() === 'ltr'"
                                [attr.aria-pressed]="currentDirection() === 'ltr'"
                ...
                <div *ngIf="!simple && location === 'app'">
                    <div class="flex flex-col gap-2">
                        <span class="text-lg text-muted-color font-semibold">نوع القائمة</span>
                        <div class="flex flex-wrap flex-col gap-3">
                            <div class="flex">
                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="static" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('static')" inputId="static"></p-radio-button>
                                    <label for="static">ثابتة</label>
                                </div>

                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="overlay" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('overlay')" inputId="overlay"></p-radio-button>
                                    <label for="overlay">تراكب</label>
                                </div>
                            </div>
                            <div class="flex">
                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="slim" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('slim')" inputId="slim"></p-radio-button>
                                    <label for="slim">رفيعة</label>
                                </div>
                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="compact" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('compact')" inputId="compact"></p-radio-button>
                                    <label for="compact">مضغوطة</label>
                                </div>
                            </div>
                            <div class="flex">
                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="reveal" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('reveal')" inputId="reveal"></p-radio-button>
                                    <label for="reveal">كشف</label>
                                </div>
                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="drawer" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('drawer')" inputId="drawer"></p-radio-button>
                                    <label for="drawer">درج</label>
                                </div>
                            </div>
                            <div class="flex">
                                <div class="flex items-center gap-2 w-6/12">
                                    <p-radio-button name="menuMode" value="horizontal" [ngModel]="layoutService.layoutConfig().menuMode" (ngModelChange)="setMenuMode('horizontal')" inputId="horizontal"></p-radio-button>
                                    <label for="horizontal">أفقية</label>
                                </div>
                            </div>
                        </div>
                    </div>
...
export class AppConfigurator implements OnInit {
    ...
    currentDirection = computed<'ltr' | 'rtl'>(() => this.layoutService.layoutConfig().direction);
